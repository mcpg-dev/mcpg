...
| `mcpg-plugin-cluster-{nats,redis}` | `libs/plugins/cluster/*/` | Cluster cdylibs. Selected via top-level `cluster: { kind: ..., ...config }`. The cluster plugin internally instantiates the four primitive impls and exposes them via the trait's `key_value_store()` / `pub_sub()` / `lease()` / `watch()` accessors. |
